...
Fixes the crash on SP1 activation. The handle copy constructor receives a source whose `+0x40` control block is invalid; **only that field** is neutralised, never the payload at `+0x38` (see the 2026-08-22 correction below). Validity criteria: non-null, 8-byte aligned, above `0x100000`, and no high bits beyond bit 47.
